...
func MigrateCosmosOrDie(ctx context.Context, resourcesDBClient database.ResourcesDBClient) {
	...
	for _, subscription := range subscriptionIterator.Items(ctx) {
		...
		if err != nil {
			logger.Error(err, "failed to list operations", "subscription", subscription.ResourceID)
			panic(err)
		}
		retrievedOperationsCount := 0
		for _, operation := range operationIterator.Items(ctx) {
			retrievedOperationsCount++
			_, err := resourcesDBClient.Operations(operation.ResourceID.SubscriptionID).Get(ctx, operation.ResourceID.Name)
			if err != nil {
				logger.Error(err, "failed to get operation", "subscription", subscription.ResourceID, "operation", operation.ResourceID, "operationsRetrieved", retrievedOperationsCount)
				panic(err)
			}

		}
		if err := operationIterator.GetError(); err != nil {
			logger.Error(err, "failed to iterate operations", "subscription", subscription.ResourceID, "operationsRetrieved", retrievedOperationsCount)
			panic(err)
		}
		logger.Info(fmt.Sprintf("retrieval of operations completed. Retrieved %d operations", retrievedOperationsCount), "subscription", subscription.ResourceID)
